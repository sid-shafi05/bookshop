...
router.get('/:wishlist_id', async (req, res) => {
    ...
    try {
        ...
        res.json({ books: result.rows, total: result.rows.length, wishlist_id: wishlist_id });
    } catch (err) {
...
router.post('/add', async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length === 0) {
            return res.json({ added: false, message: 'Book already exists in this wishlist.' });
        }

        res.status(201).json({ added: true, message: 'Book saved to wishlist successfully!', item: result.rows[0] });
    } catch (err) {
...
router.delete('/:wishlist_id', async (req, res) => {
    ...
    try {
        ...
        if (Number(check.rows[0].customer_id) !== req.userId) {
            return res.status(403).json({ error: 'Access denied: not your wishlist' });
        }
